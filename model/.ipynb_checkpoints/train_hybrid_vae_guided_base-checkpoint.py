...
class HybridGuidedVAETrainer():
    def __init__(self, param_file, dataset_path):
        ...
        self.generate_data_batch(dataset, dataset_path)
        ...
        self.net = VAE(input_shape=self.params['input_shape'], seq_len=self.params['chunk_size_train'], dimz=self.params['dimz'], encoder_params=self.params).to(self.device)
        ...
        if self.args.resume_from != 'None':
            ...
            if not os.path.exists(self.checkpoint_dir) and not args.no_save:
    ...
    def generate_data_batch(self, dataset, dataset_path):
        ...
        self.train_dl, self.test_dl = create_data(
                                      ...
                                      time_shuffle=self.params['time_shuffle'])#True)#True)
    ...
    def train_step_guided(self, s, x, t, vae_beta=1): # t is the target class
        ...
        #print("INHIB SHAPE",inhib_z.shape)
